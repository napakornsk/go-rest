...
func (h *PortfolioHandler) CreatePersonalProjectHandler(c *gin.Context) {
	req := new(model.PersonalProject)
	err := c.BindJSON(req)
	if err != nil {
	...
	res, err := h.srv.CreatePersonalProject(req)
	if err != nil {
...
func (h *PortfolioHandler) GetPersonalProjectHandler(c *gin.Context) {
	...
	data, err := h.srv.GetPersonalProjectById(&userId)
	if err != nil {
		log.Printf("Failed to receive data from service: %v\n", err)
		c.IndentedJSON(
			http.StatusInternalServerError,
