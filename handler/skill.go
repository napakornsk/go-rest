...
func (h *PortfolioHandler) CreateSkillHandler(c *gin.Context) {
	req := new(model.Skill)
	err := c.BindJSON(req)
	if err != nil {
...
func (h *PortfolioHandler) GetSkillHandler(c *gin.Context) {
	...
	data, err := h.srv.GetSkillById(&userId)
	if err != nil {
		log.Printf("Failed to receive data from service: %v\n", err)
		c.IndentedJSON(
			http.StatusInternalServerError,
